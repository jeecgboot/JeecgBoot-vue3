  ...
  setup(props) {
    ...
    const getColumns = computed(() => {
      ...
      for (const key in dataSource[0]) {
        summaryColumns.add(key);
      }

      // 将未参与合计计算的列的render去除，防止渲染列设置中的customRender
      for (let index = 0; index < columns.length; index++) {
        const col = columns[index];
        const hasColumn = summaryColumns.has(col.key || col.dataIndex);
        // if (!hasColumn && col.hasOwnProperty('customRender')) {
        //   Reflect.deleteProperty(col, 'customRender');
        // }
        if (hasColumn && col.hasOwnProperty('customSummaryRender')) {
          // col.customRender = col.customSummaryRender
        }
      }
